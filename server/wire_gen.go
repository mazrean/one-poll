...
func InjectService(config *Config) (*Service, error) {
	...
	webAuthnRelyingParty := config.RelyingParty
	webAuthnCredential, err := gorm2.NewWebAuthnCredential(db)
	if err != nil {
		return nil, err
	}
	webAuthn, err := v1_2.NewWebAuthn(webAuthnRelyingParty, db, webAuthnCredential)
	if err != nil {
		return nil, err
	}
	v1WebAuthn := v1.NewWebAuthn(session, webAuthn)
	api := v1.NewAPI(checker, v1User, poll2, tag2, comment2, response2, v1WebAuthn)
...
var (
	...
	webauthnServiceBind      = wire.Bind(new(service.WebAuthn), new(*v1_2.WebAuthn))
)
